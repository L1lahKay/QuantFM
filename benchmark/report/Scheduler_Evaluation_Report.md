...
| 性能赢家 | **尚不能判定跨调度器性能赢家。** 当前已完成 `15/15` 个矩阵单元，存在真实 Training Time 与 GPU utilization，并已形成同指纹 Scaling Efficiency；本矩阵不能据此做三调度器性能排名。 |
...
| 当前执行状态 | host GPU compute process 的最新只读快照为 **0**；该易变 gate 当前已解除；current-safe 本批次按 synthetic_ephemeral 契约完成，不挂 PVC/hostPath；使用 512Mi memory-backed emptyDir；一次性授权不可复用；本性能矩阵已完成；后续高级调度实验仍需新的专项授权、逐次 idle-GPU/quota preflight 与监控链。 |
...
> 生成时间：`2026-08-06T06:42:18Z`  
> 报告性质：**COMPLETED MATRIX：显式验收矩阵的 cell 与重复次数契约均已有 completed 证据；仍须通过能力门槛。**  
> 验收矩阵：`current-safe`，来源 `/home/khalil/DataCleaning7.3/QuantFM/benchmark/config/current-safe-matrix.json`；5 scenarios × 3 schedulers = 15 cells。  
> 矩阵边界：这是当前四卡配额边界内的工程矩阵，不是 requested-target；其完成度不得外推为 8-GPU/压力测试目标完成度。 本矩阵已获一次性 synthetic_ephemeral 授权：输入在进程内生成，Pod 不挂 PVC/hostPath，只使用 512Mi memory-backed emptyDir，且禁止下载、checkpoint、持久缓存和 durable output。  
> Cell 覆盖度：`15/15`；重复次数完成度：`45/45`（每格要求 N=3）；实际训练/提交捕获：`63`（completed/blocked/failed：`60/0/3`）；未完成矩阵单元：`0`。  
> 清理验收：`60/60` 个 completed run 同时满足精确对象不存在、owner UID 依赖不存在和 GPU quota 恢复。  
> 执行阶段：completed=`60`，submitted-incomplete=`3`，server-dry-run=`0`，readiness-only=`0`。dry-run/readiness **不计为**模型实验完成。  
...
| 当前矩阵 storage mode | `synthetic_ephemeral` | 本批次证据契约为无 PVC/hostPath、512Mi memory-backed emptyDir，禁止下载/checkpoint/持久缓存/durable output；不代表未来运行权限 |
...
| 最新只读 host process snapshot | at=2026-08-06T06:23:13.395Z, process=0, GPU UUID=N/A, memory≈N/A MiB | `/home/khalil/DataCleaning7.3/QuantFM/benchmark/results/raw/khalil-bm-k8s-9400c4-nn-single-o36604f7b-c015-r02/preflight-host-gpu-processes.json`；这是当前易变状态 |
...
- 本报告只以 `current-safe` 的 JSON 内容作为完成度分母；不会隐式追加另一个内置矩阵的单元。
- 矩阵存储模式：`synthetic_ephemeral`；本批次证据契约不挂载 PVC/hostPath，所有 scratch 位于 512Mi memory-backed emptyDir；报告不把历史一次性授权解释为未来运行权限。

| Scenario ID | Model | Mode | GPU | Matrix execution |
|---|---|---|---:|---|
| nn-single-gpu1 | NN | Single Pod | 1 | allowed |
| nn-multipod-gpu4 | NN | Multi Pod | 4 | allowed |
| transformer-single-gpu1 | Transformer | Single Pod | 1 | allowed |
| transformer-single-gpu4 | Transformer | Single Pod | 4 | allowed |
| transformer-multipod-gpu4 | Transformer | Multi Pod | 4 | allowed |
...
矩阵总完成度为 **15/15**。API dry-run 和历史功能测试不进入 completed training 分子。模型指标的 15-cell 诊断矩阵已移至文末附录，避免用重复的 `N/A/blocked` 淹没已验证结论。
...
| Quota | N/A | Historical N=1: nominal GPU quota, 7 个物理 GPU 空闲时 Workload 仍等待 | Historical N=1: Queue GPU capability wait/release |
...
| Multi GPU | Observed — 16 个完成的多 GPU run（仅证明执行链）；独立 NCCL probe=contaminated | Observed — 9 个完成的多 GPU run（仅证明执行链） | Observed — 9 个完成的多 GPU run（仅证明执行链） |
...
以上 `prior N=1 functional test` 来自 2026-07-31 的调度功能评估 JSON：[kueue](../../docs/assets/gpu-scheduler-evaluation/current-kueue-results.json), [volcano](../../docs/assets/gpu-scheduler-evaluation/volcano-results.json)。它们验证调度语义，不是 LGB/NN/Transformer 训练性能样本，未写入本轮 throughput/scaling 计算。
...
|---|---|---:|---:|
| Bare K8s | ResourceQuota waiter | N/A | N/A |
| Kueue | GPU/CUDA admission | 9.000 | 8.934 |
| Kueue | quota waiter | 37.000 | 37.155 |
| Kueue | all-or-nothing admission | 21.000 | 21.669 |
| Kueue | preemptor | 15.000 | 15.625 |
...
- 必测 workload 矩阵完成 `15/15`；仍有 `0` 个单元没有 completed run。
...
|---|---|---|---:|---|---:|---:|---:|---:|
| nn-single-gpu1 | K8s | Single Pod | 1 | completed | 0.316 | 11.390 | 5.00 | 100.00 |
| nn-single-gpu1 | Kueue | Single Pod | 1 | completed | 0.285 | 11.328 | 13.00 | 100.00 |
| nn-single-gpu1 | Volcano | Single Pod | 1 | completed | 0.313 | 12.168 | 12.00 | 100.00 |
| nn-multipod-gpu4 | K8s | Multi Pod | 4 | completed | 1.152 | 21.621 | 78.38 | 6.82 |
| nn-multipod-gpu4 | Kueue | Multi Pod | 4 | completed | 0.990 | 20.333 | 68.75 | 7.36 |
| nn-multipod-gpu4 | Volcano | Multi Pod | 4 | completed | 1.136 | 21.616 | 77.75 | 6.50 |
| transformer-single-gpu1 | K8s | Single Pod | 1 | completed | 7.834 | 23.996 | 12.86 | 100.00 |
| transformer-single-gpu1 | Kueue | Single Pod | 1 | completed | 8.029 | 21.711 | 12.86 | 100.00 |
| transformer-single-gpu1 | Volcano | Single Pod | 1 | completed | 8.079 | 22.873 | 12.29 | 100.00 |
| transformer-single-gpu4 | K8s | Single Pod | 4 | completed | 9.332 | 29.126 | 39.00 | 21.30 |
| transformer-single-gpu4 | Kueue | Single Pod | 4 | completed | 9.129 | 26.928 | 41.61 | 21.99 |
| transformer-single-gpu4 | Volcano | Single Pod | 4 | completed | 9.121 | 28.760 | 36.27 | 21.79 |
| transformer-multipod-gpu4 | K8s | Multi Pod | 4 | completed | 25.448 | 47.237 | 73.05 | 7.81 |
| transformer-multipod-gpu4 | Kueue | Multi Pod | 4 | completed | 26.196 | 49.325 | 67.58 | 7.66 |
| transformer-multipod-gpu4 | Volcano | Multi Pod | 4 | completed | 34.698 | 59.093 | 82.38 | 5.73 |
...
- `nn-single-gpu1` 已覆盖矩阵声明的 3 个 scheduler；本次最短 training time 为 `Kueue 0.285s`。单次描述结果仍不能独立证明生产优势。
- `nn-multipod-gpu4` 已覆盖矩阵声明的 3 个 scheduler；本次最短 training time 为 `Volcano 0.964s`。单次描述结果仍不能独立证明生产优势。
- `transformer-single-gpu1` 已覆盖矩阵声明的 3 个 scheduler；本次最短 training time 为 `K8s 7.809s`。单次描述结果仍不能独立证明生产优势。
- `transformer-single-gpu4` 已覆盖矩阵声明的 3 个 scheduler；本次最短 training time 为 `Kueue 8.802s`。单次描述结果仍不能独立证明生产优势。
- `transformer-multipod-gpu4` 已覆盖矩阵声明的 3 个 scheduler；本次最短 training time 为 `K8s 25.448s`。单次描述结果仍不能独立证明生产优势。

## 诊断附录 B：原始尝试与缺口数据

- 捕获尝试总数：`63`；completed=`60`，server-dry-run=`0`，readiness-only=`0`。
