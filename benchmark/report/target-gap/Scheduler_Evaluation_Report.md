...
| 性能赢家 | **尚不能判定。** 当前可执行 GPU 模型矩阵没有 completed training，不能比较 Training Time、GPU utilization 或 Scaling Efficiency。 |
...
| 当前执行状态 | host GPU compute process 的最新只读快照为 **0**；该易变 gate 当前已解除；所选矩阵要求持久存储，现有 quantfm-data 位于根盘，非根盘 PVC gate 未通过；剩余 gate 是 根盘 PVC、Kueue LocalQueue/Volcano 测试 Queue 与授权、监控链。 |
...
| Q2 Single Pod 与 Multi Pod 性能差异？ | **未形成性能结论。** 4-GPU NCCL 探针只证明 collective 可运行，且其捕获时受外部进程污染。 | 探针不能替代 NN/Transformer completed training，也不能计算 scaling efficiency。 |
...
当前矩阵没有匹配的完整 server-side dry-run summary；此项不会被记作通过。
...
> 生成时间：`2026-08-03T08:39:42Z`  
> 报告性质：**FUNCTIONAL EVIDENCE + READINESS：已有 N=1 调度功能实测和 wall-clock；没有 completed 模型训练，因此架构结论有效但性能赢家待测。**  
> 验收矩阵：`requested-target`，来源 `/home/khalil/DataCleaning7.3/QuantFM/benchmark/config/target-matrix.json`；9 scenarios × 3 schedulers = 27 cells。  
> 矩阵边界：这是 requested-target 矩阵（含其中声明的禁用/压力场景），不与 current-safe 完成度合并。  
> 矩阵完成度：`0/27`；实际捕获 run：`22`（completed/blocked/failed：`0/22/0`）；未完成矩阵单元：`27`。  
> 执行阶段：completed=`0`，submitted-incomplete=`0`，server-dry-run=`13`，readiness-only=`9`。dry-run/readiness **不计为**模型实验完成。  
...
| 当前矩阵 storage mode | `persistent_pvc` | 持久路径必须通过非根盘 PVC gate |
...
| 最新只读 host process snapshot | at=2026-08-03T07:40:09.711Z, process=0, GPU UUID=N/A, memory≈N/A MiB | `/home/khalil/DataCleaning7.3/QuantFM/benchmark/results/raw/khalil-bm-k8s-d08b90-transform-o480a428b-c015-r03/preflight-host-gpu-processes.json`；这是当前易变状态 |

只读清点显示当前是 **1 个 GPU 节点、该节点 8 张 GPU**，不是“8 个 GPU 节点”。同时 `gpu-dev` GPU hard quota 为 4，所以本轮 8-GPU 单元在不绕过配额的前提下不可执行，应保持 blocked。

`findmnt -T /data` 与 `findmnt -T /` 均指向 `/dev/mapper/ubuntu--vg-ubuntu--lv`：现有 `quantfm-data` local-path 实际位于根文件系统。这是所选持久训练矩阵的**硬阻塞**，不是单纯缺少口头确认；应先由管理员提供并确认真正独立磁盘或 CSI 后端的 PVC。
...
- 本报告只以 `requested-target` 的 JSON 内容作为完成度分母；不会隐式追加另一个内置矩阵的单元。
- 矩阵存储模式：`persistent_pvc`；持久训练仍须通过已确认的非根盘 PVC gate。

| Scenario ID | Model | Mode | GPU | Matrix execution |
|---|---|---|---:|---|
| lgb-single-cpu | LGB | Single Pod | 0 | disabled — The CPU runner path exists, but no observed digest-pinned internal LightGBM image has passed the in-container CPU training smoke; storage approval is also still blocked. |
| lgb-single-gpu1 | LGB | Single Pod | 1 | disabled — No digest-pinned LightGBM build has passed an in-container device_type=cuda smoke; wheel provenance alone does not establish the requested CUDA backend, so this GPU cell remains blocked. |
| nn-single-gpu1 | NN | Single Pod | 1 | allowed |
| nn-multipod-gpu4 | NN | Multi Pod | 4 | allowed |
| transformer-single-gpu8 | Transformer | Single Pod | 8 | disabled — Requires eight GPUs but gpu-dev has a hard four-GPU quota and the runner refuses requests above four. |
| transformer-multipod-gpu8 | Transformer | Multi Pod | 8 | disabled — Requires eight GPUs but gpu-dev has a hard four-GPU quota and the runner refuses requests above four. |
| production-contention-a-transformer8 | Transformer | Multi Pod | 8 | disabled — The requested concurrent 8+2 GPU pressure test exceeds both the four-GPU namespace quota and the runner safety ceiling. |
| production-contention-b-nn2 | NN | Single Pod | 2 | disabled — This row is part of the requested concurrent A(8 GPU)+B(2 GPU)+C(CPU) pressure group, whose aggregate GPU demand is unsafe under the four-GPU quota. |
| production-contention-c-lgb-cpu | LGB | Single Pod | 0 | disabled — CPU-only pressure Jobs need a separate bounded policy and this row belongs to the unsafe 8+2 GPU concurrent pressure group. |
...
矩阵总完成度为 **0/27**。API dry-run 和历史功能测试不进入 completed training 分子。模型指标的 27-cell 诊断矩阵已移至文末附录，避免用重复的 `N/A/blocked` 淹没已验证结论。

### 当前需要解除的全局 gate

| Gate | 当前状态 | 对真实训练的影响 |
|---|---|---|
| 训练存储 | `quantfm-data` 是与 `/` 同盘的 local-path PVC | 阻止所选真实训练提交 |
| Kueue | 2026-08-03 bounded authorization 已激活；`gpu-dev` LocalQueue 尚未建立/观测为 Active | Queue setup 完成前，Kueue 单元不能形成新的 quota Admission 证据 |
| Volcano | 2026-08-03 current-safe Queue/PodGroup 例外已激活；`khalil-volcano-smoke` Queue 尚未建立/观测为 Open | Queue setup 完成前，Volcano webhook 拒绝测试 manifest，不能形成新的 Gang 训练证据 |
| 监控 | GPU Operator/DCGM/Prometheus/Grafana 未检测到 | 不阻止 server dry-run，但阻止最终监控交付与正式评估 |
| Host GPU process | 最新只读快照 process count=0 | 当前无此阻塞；每次性能 run 仍须重新检查 |
| LightGBM/8-GPU gap | 不属于 current-safe release 分母 | 保留在 requested-target gap，不再让主矩阵永久不可完成 |
...
| Preemption | N/A current safe run：namespace quota 阻止构造 node-pressure victim；不能据历史不完整样本判定 | Historical N=1: victim Evicted → high-priority Workload started；current gpu-dev LocalQueue=absent | Historical N=1 under temporary `preempt` action: victim → preemptor；current preempt=false, reclaim=false |
...
| Multi GPU | Partial: one-Pod/4-GPU NCCL collective passed (contaminated functional probe)；无模型训练或 multi-Pod | Partial (prior N=1): 2-GPU PodSet scheduled；无 NCCL/DDP | Partial (prior N=1): 2-member/2-GPU Gang；无 NCCL/DDP |
...
**性能选型结论：未形成。当前产物是 readiness/server-side dry-run 诊断，不是最终性能评估。**

- 真实 completed 模型训练为 `0`；`13` 个 server dry-run 和 `9` 个 readiness-only run 只证明前置检查/Admission 路径，不能提供 training time、GPU utilization、throughput 或调度器排名。
- 显式 `requested-target` 矩阵完成 `0/27`；仍有 `27` 个单元没有 completed run。
- 因此本节只列修复顺序和未来决策规则，不指定 Native K8s、Kueue 或 Volcano 为生产赢家。
- 现有 `quantfm-data` 位于根文件系统，不满足强制非根盘 PVC gate；必须先迁移到独立磁盘/CSI PVC 并由管理员确认。
...
**能力/风险层架构结论：CAPABILITY-BASED PROVISIONAL（不是性能实测赢家）。**

- 生产默认候选：由 **Kueue** 负责普通多租户 Job 的 admission/quota/priority，实际 Pod 继续交给 `default-scheduler`；当前集群缺少 LocalQueue/fair-sharing 实配，因此须补齐配置与验收后才能上线。
- 严格 gang、整组启动和分布式训练进入隔离的 **Volcano scheduler + PodGroup lane**；只允许已经通过真实 DDP/NCCL、故障恢复和租户隔离验收的 workload 使用。
- 同一个 workload 必须只有一个 admission/queue owner；不得让 Kueue 和 Volcano 对同一 Job 双重准入或双重队列控制。
- Native K8s 仅保留给简单独立任务和性能基线，ResourceQuota 不能替代批队列。
- Transformer 并非天然必须 Volcano；是否进入 Volcano lane 取决于 gang/整组启动需求及 Kueue TAS + `waitForPodsReady` 与 Volcano PodGroup 的同负载验证结果。
...
- 未验证的数据集规模、模型收敛质量、跨节点网络带宽、存储吞吐和 checkpoint 恢复不能从短时 smoke workload 推断。
- 若 8 GPU 请求超过 namespace ResourceQuota、每 Job GPU 上限或实际单节点容量，该矩阵单元应保持 blocked，不能绕过配额执行。
- 现有 local-path PVC 与 `/` 同一文件系统；在真正独立盘/CSI PVC 配置并确认前，不得提交持久训练或把根盘用于 benchmark 持久数据。
...
|---|---|---|---:|---|---:|---:|---:|---:|
| lgb-single-cpu | K8s | Single Pod | 0 | not-run | N/A | N/A | N/A | N/A |
| lgb-single-cpu | Kueue | Single Pod | 0 | not-run | N/A | N/A | N/A | N/A |
| lgb-single-cpu | Volcano | Single Pod | 0 | not-run | N/A | N/A | N/A | N/A |
| lgb-single-gpu1 | K8s | Single Pod | 1 | not-run | N/A | N/A | N/A | N/A |
| lgb-single-gpu1 | Kueue | Single Pod | 1 | not-run | N/A | N/A | N/A | N/A |
| lgb-single-gpu1 | Volcano | Single Pod | 1 | not-run | N/A | N/A | N/A | N/A |
| nn-single-gpu1 | K8s | Single Pod | 1 | server-dry-run | N/A | N/A | N/A | N/A |
| nn-single-gpu1 | Kueue | Single Pod | 1 | server-dry-run | N/A | N/A | N/A | N/A |
| nn-single-gpu1 | Volcano | Single Pod | 1 | server-dry-run | N/A | N/A | N/A | N/A |
| nn-multipod-gpu4 | K8s | Multi Pod | 4 | server-dry-run | N/A | N/A | N/A | N/A |
| nn-multipod-gpu4 | Kueue | Multi Pod | 4 | server-dry-run | N/A | N/A | N/A | N/A |
| nn-multipod-gpu4 | Volcano | Multi Pod | 4 | server-dry-run | N/A | N/A | N/A | N/A |
| transformer-single-gpu8 | K8s | Single Pod | 8 | not-run | N/A | N/A | N/A | N/A |
| transformer-single-gpu8 | Kueue | Single Pod | 8 | not-run | N/A | N/A | N/A | N/A |
| transformer-single-gpu8 | Volcano | Single Pod | 8 | not-run | N/A | N/A | N/A | N/A |
| transformer-multipod-gpu8 | K8s | Multi Pod | 8 | not-run | N/A | N/A | N/A | N/A |
| transformer-multipod-gpu8 | Kueue | Multi Pod | 8 | not-run | N/A | N/A | N/A | N/A |
| transformer-multipod-gpu8 | Volcano | Multi Pod | 8 | not-run | N/A | N/A | N/A | N/A |
| production-contention-a-transformer8 | K8s | Multi Pod | 8 | not-run | N/A | N/A | N/A | N/A |
| production-contention-a-transformer8 | Kueue | Multi Pod | 8 | not-run | N/A | N/A | N/A | N/A |
| production-contention-a-transformer8 | Volcano | Multi Pod | 8 | not-run | N/A | N/A | N/A | N/A |
| production-contention-b-nn2 | K8s | Single Pod | 2 | not-run | N/A | N/A | N/A | N/A |
| production-contention-b-nn2 | Kueue | Single Pod | 2 | not-run | N/A | N/A | N/A | N/A |
| production-contention-b-nn2 | Volcano | Single Pod | 2 | not-run | N/A | N/A | N/A | N/A |
| production-contention-c-lgb-cpu | K8s | Single Pod | 0 | not-run | N/A | N/A | N/A | N/A |
| production-contention-c-lgb-cpu | Kueue | Single Pod | 0 | not-run | N/A | N/A | N/A | N/A |
| production-contention-c-lgb-cpu | Volcano | Single Pod | 0 | not-run | N/A | N/A | N/A | N/A |
...
本矩阵当前没有任何 scenario 同时具备三个调度器的 completed training，因此本附录不生成性能排名。

## 诊断附录 B：原始尝试与缺口数据

- 捕获尝试总数：`22`；completed=`0`，server-dry-run=`13`，readiness-only=`9`。
