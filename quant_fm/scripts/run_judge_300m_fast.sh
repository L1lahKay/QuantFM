...
#   SKIP_EMB=1 bash ...      # 复用已合并 embeddings
#
...
echo "======== $(date -Is) judge-300m-fast (NPROC=$NPROC BATCH=$BATCH DTYPE=$DTYPE) ========"

if [[ "$SKIP_EMB" != "1" ]]; then
  for split in train val test; do
    out="$EMB_DIR/${split}.parquet"
    if [[ -f "$out" && "${FORCE_EMB:-0}" != "1" ]]; then
      echo "==> skip $split (exists: $out)"
      continue
    fi
...
allp.write_parquet(emb / "all.parquet")
print(f"wrote {emb/'all.parquet'} rows={allp.height}")
PY
else
  echo "==> SKIP_EMB=1"
fi
